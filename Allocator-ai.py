...
def should_follow(whale):
    ...
    Uses Moralis bootstrap filters (ROI %, USD profit, trade count)
    plus live allocator metrics (local ROI, winrate, score).
    """

    stats = WHALE_SCORES.get(whale, None)
    if not stats:
        return True   # no info yet, default yes

    # grab config thresholds if defined, else sane defaults
    roi_floor   = Decimal(str(config.get("min_moralis_roi_pct", 0)))
    usd_floor   = Decimal(str(config.get("min_moralis_profit_usd", 0)))
    trade_floor = config.get("min_moralis_trades", 0)

    # Moralis bootstrap pre‑filter
    if stats.get("moralis_bootstrap", False):
        if Decimal(str(stats.get("moralis_roi_pct", 0))) < roi_floor:
            return False
        if Decimal(str(stats.get("roi_usd", 0))) < usd_floor:
            return False
        if stats.get("trades", 0) < trade_floor:
            return False

    # Runtime culling
    if stats.get("roi", 0) < 0:
        return False
    if stats.get("winrate", 0) < 0.4:
        return False
    if stats.get("score", 0) < 0:
        return False
...
def setup_web3():
    ...
    w3 = Web3(Web3.LegacyWebSocketProvider(config["web3_rpc"]))   #New
...
def parse_v2_swaps(receipt, path=None):
    ...
    for log in receipt.logs:
        try:
            ...
            if len(log['topics']) >= 3 and log['topics'][0].hex() == Web3.keccak(text="Swap(address,uint256,uint256,uint256,uint256,address)").hex():
...
def parse_v3_swaps(receipt, path=None):
    ...
    for log in receipt.logs:
        try:
            ...
            #if len(log['topics']) >= 3 and log['topics'][0].hex() == Web3.keccak(text="Swap(address,address,int256)").hex()
            if len(log['topics']) >= 3 and log['topics'][0].hex() == Web3.keccak(text="Swap(address,address,int256,int256,uint160,uint128,int24)").hex():
...
<head>
    ...
    <style>
        ...
        .whale-risk-high       { background-color: rgba(220,53,69,0.15); }
        .whale-risk-medium     { background-color: rgba(255,193,7,0.15); }
        .whale-risk-profitable { background-color: rgba(40,167,69,0.15); }
...
<body>
    ...
    <table>
        ...
        {% for w in whales %}
        <tr class="
            {% if w.moralis_roi is not none and w.moralis_roi >= config.get('min_moralis_roi_pct', 20) %}
                whale-risk-profitable
...
@app.route("/")
def index():
    """Render the dashboard with Allocator+Moralis whale data"""
    # Calculate stats
    ...
    for whale in tracked_whales:
        pnl = my_pnl.get(whale, 0)
        whale_stats = WHALE_SCORES.get(whale, {})
        whale_trades = [t for t in trade_history if t["whale"] == whale]

        rm = risk_mult.get(whale, Decimal("1.0"))  # enforce Decimal default

        whale_data.append({
            "address": whale,
            "pnl": float(pnl),
            "risk": float(rm),
            "allocation": float(capital * BASE_RISK * rm),
            "count": len(whale_trades),
            "score": float(whale_stats.get("score", Decimal("0"))),
            "winrate": float(whale_stats.get("winrate", 0) * 100)
                       if "winrate" in whale_stats else 0,
            # FIXED: pull Moralis ROI from its own field
            "moralis_roi": float(whale_stats.get("moralis_roi_pct", 0))
                           if whale_stats.get("moralis_bootstrap") else None,
            "moralis_profit_usd": float(whale_stats.get("roi_usd", 0))
                                  if whale_stats.get("moralis_bootstrap") else None,
            "moralis_trades": whale_stats.get("trades", 0)
                              if whale_stats.get("moralis_bootstrap") else None
        })



    # Prepare whale data (Allocator stats + Moralis bootstrap if available)
    ##whale_data = []
    ...
    ##    whale_trades = [t for t in trade_history if t["whale"] == whale]
        ...
        ##whale_data.append({
        ##    "address": whale,
        ##    # Allocator live stats
        ##    "pnl": float(pnl),
        ##    "risk": float(risk_mult[whale]),
        ##    "allocation": float(capital * BASE_RISK * risk_mult[whale]),
        ##    "count": len(whale_trades),
        ##    "score": float(whale_stats.get("score", 0)),
        ##    "winrate": float(whale_stats.get("winrate", 0) * 100)
        ##                if "winrate" in whale_stats else 0,
        ##    # Moralis bootstrap stats (optional)
        ##    "moralis_roi": float(whale_stats.get("score", 0))
        ##                   if whale_stats.get("moralis_bootstrap") else None,
        ##    "moralis_profit_usd": float(whale_stats.get("roi_usd", 0))
        ##                          if whale_stats.get("moralis_bootstrap") else None,
        ##    "moralis_trades": whale_stats.get("trades", 0)
...
from config import DISCOVERY_MODE, DISCOVERY_MODES

from datetime import datetime, timedelta

def closed_position_within(actor_stats, window_hours=72):
    """
    Placeholder: check if an actor closed a profitable round-trip 
    inside the last `window_hours`.
    Right now it's just a stub that always returns True 
    if profit > 0 (you'll want to replace with actual trade ledger logic).
    """
    # TODO: wire in per-token entry/exit matching.
    now = datetime.utcnow()
    cutoff = now - timedelta(hours=window_hours)
    # actor_stats could carry timestamps of entry + exit 
    # (extend candidate_stats to store them).
    return actor_stats["profit"] > 0


def discover_whales(mode: str = DISCOVERY_MODE, simulate: bool = False, w3=None):
    """
    Crawl recent blocks, find whale candidates using configured sieve.
    Modes: bot_hunter / active_whale / lazy_whale / quick_profit_whale
    Only call Moralis bootstrap the FIRST time we see a new whale.
    Already validated whales are cached in WHALE_SCORES → no CU burn.
    """

    logger.info(f"[DEBUG] >>> entered discover_whales v2 with mode={mode}, simulate={simulate}")

    # if no w3 explicitly passed, fall back to the global
    w3 = w3 or globals().get("w3")
    if w3 is None:
        raise RuntimeError("No Web3 connection provided")

    params = DISCOVERY_MODES.get(mode)
    if not params:
        logger.warning(f"[Discovery] Unknown mode={mode}, falling back to active_whale")
        params = DISCOVERY_MODES["active_whale"]

    blocks_back   = params["blocks_back"]
    min_trades    = params["min_trades"]
    min_pnl_thr   = params["min_pnl_threshold"]
    profit_window = params.get("profit_window_hours")

    logger.info(f"[Discovery] Mode={mode} → "
                f"back {blocks_back} blocks, trades≥{min_trades}, pnl≥{min_pnl_thr} ETH")
    ...
    # -------- Stage 1: crude on‑chain sieve --------
    block_count, tx_count = 0, 0
    for i in range(start_block, end_block + 1):
        try:
            ...
            # light heartbeat every 50 blocks (debug only)
            if i % 50 == 0:
                logger.debug(f"[{mode}] scanned block {i} → {len(txs)} txs")
            ...
            logger.debug(f"[{mode}] failed to pull block {i}: {e}")
            continue

    logger.info(f"[{mode}] sieve summary → scanned {block_count} blocks, {tx_count} txs, "
                f"candidates={len(candidate_stats)}")

    # preview first 20 candidate stats
    for addr, stats in list(candidate_stats.items())[:20]:
        logger.debug(f"[DEBUG] candidate {addr} → trades={stats['trades']} pnl={stats['profit']}")

    # -------- Stage 2 filter by mode --------
    new_whales = []
    for addr, stats in candidate_stats.items():
        if stats["trades"] < min_trades or stats["profit"] < min_pnl_thr:
            continue
        if mode == "quick_profit_whale":
            if not closed_position_within(stats, window_hours=profit_window):
                continue
        new_whales.append(addr)

    logger.info(f"[Discovery] pre‑filter={len(candidate_stats)}, post‑filter={len(new_whales)}")
    logger.info(f"[Discovery] {len(new_whales)} candidate whales → checking Moralis (only unseen).")

    if simulate:
        logger.info(f"[Simulation] Would’ve flagged {len(new_whales)} whales under {mode}")
        for w in new_whales[:20]:  # preview first 20
            logger.info(f"  {w} → trades={candidate_stats[w]['trades']}, "
                        f"pnl={candidate_stats[w]['profit']}")
        return new_whales

    # -------- Stage 2: Moralis enrichment / DB cache --------
    headers = {"X-API-Key": MORALIS_API_KEY}
    for w in new_whales:
        if w in tracked_whales:
            continue

        db_row = get_whale_from_db(w)
        needs_refresh = True

        if db_row:
            _, roi_pct, usd, trades, _, last_refresh = db_row
            last_refresh = last_refresh or 0
            if time.time() - last_refresh < REFRESH_INTERVAL:
                WHALE_SCORES[w].update({
                    "score": Decimal("0"),
                    "moralis_roi_pct": Decimal(str(roi_pct)),
                    "roi_usd": Decimal(str(usd)),
                    "trades": trades,
                    "moralis_bootstrap": True
                })
                tracked_whales.add(w)
                logger.info(f"[Cache] Whale {w} loaded from DB "
                            f"(ROI {roi_pct}%, ${usd}, {trades} trades)")
                needs_refresh = False

        if not needs_refresh:
            continue

        # Call Moralis if fresh required
        try:
            url = f"{MORALIS_BASE}/wallets/{w}/profitability/summary?chain=eth"
            resp = requests.get(url, headers=headers, timeout=10)
            if resp.status_code != 200:
                logger.warning(f"[Moralis] Skipped whale {w}, API {resp.status_code}")
                continue
            ...
            if (realized_usd < min_pnl_thr or realized_pct <= 0 or total_trades < min_trades):
                logger.info(f"[Moralis] Whale {w} dropped "
                            f"(profit ${realized_usd}, ROI {realized_pct}%, trades={total_trades})")
                continue

            tracked_whales.add(w)
            WHALE_SCORES[w].update({
                "score": Decimal("0"),
                "moralis_roi_pct": realized_pct,
                "roi_usd": realized_usd,
                "trades": total_trades,
                "moralis_bootstrap": True
            })

            save_whale_to_db(w, realized_pct, realized_usd, total_trades)

            logger.info(f"[Discovery] Refreshed whale {w} "
                        f"({realized_pct}% ROI, ${realized_usd}, {total_trades} trades)")

        except Exception as e:
            logger.warning(f"[Moralis] Whale {w} refresh failed: {e}")
...
def make_w3():
    return Web3(Web3.LegacyWebSocketProvider(config["web3_rpc"]))

def run_discover(mode, simulate=True):
    try:
        local_w3 = make_w3()
        discover_whales(mode, simulate=simulate, w3=local_w3)
    except Exception as e:
        logger.error(f"[{mode}] discovery crashed: {e}", exc_info=True)


def main():
    mode = 'TEST' if TEST_MODE else ('DRY_RUN' if DRY_RUN else ('DRY_RUN_MEMPOOLHACK'
                   if DRY_RUN_MEMPOOLHACK else 'LIVE'))
    logger.info(f"Starting Allocator AI in {mode} mode")

    # --- dashboard ---
    threading.Thread(target=run_dashboard, daemon=True).start()
    logger.info("[DEBUG] dashboard thread launched")
    ...
    modes = ["active_whale", "quick_profit_whale", "fast_mover_whale"]

    # --- bootstrap discovery (one full inline pass per mode) ---
    for m in modes:
        logger.info(f"[MAIN] bootstrap discovery start for {m}")
        try:
            run_discover(m)
            logger.info(f"[MAIN] bootstrap discovery finished for {m}")
        except Exception as e:
            logger.error(f"[bootstrap:{m}] crashed: {e}", exc_info=True)

    logger.info(f"[MAIN] bootstrap discovery completed for {len(modes)} modes")

    # --- mempool watcher ---
    threading.Thread(target=watch_mempool, daemon=True).start()
    logger.info("[MAIN] mempool watcher started")

    # --- periodic discovery loop ---
    def periodic_discover(interval=600):
        logger.info("[DEBUG] periodic_discover thread started")
        try:
            time.sleep(interval)  # avoid double bootstrap at t=0
            round_num = 1
            while True:
                logger.info(f"[MAIN] starting discovery refresh round {round_num}")
                threads = []
                for m in modes:
                    t = threading.Thread(target=run_discover, args=(m,), daemon=True)
                    t.start()
                    threads.append(t)
                # wait until this round’s threads finished
                for t in threads:
                    t.join()
                logger.info(f"[MAIN] finished refresh round {round_num}")
                round_num += 1
                time.sleep(interval)
        except Exception as e:
            logger.error(f"[periodic_discover] crashed: {e}", exc_info=True)

    threading.Thread(target=periodic_discover, daemon=True).start()
    logger.info("[DEBUG] launched periodic_discover thread")

    # --- keep service alive for systemd ---
    try:
        while True:
            time.sleep(3600)
